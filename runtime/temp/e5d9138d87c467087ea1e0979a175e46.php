    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="row-fluid inbox">
                <a style="margin-bottom: 5px;" class="btn blue"  href="<?php echo url('add'); ?>"><i class="icon-plus"></i>新增产品</a>
                    ...
                    <div class="portlet-body no-more-tables" style='overflow:auto'>
                        ...
                        <form action="<?php echo url('index'); ?>" id='searchForm' method='post' class="span5">
                            ...
                            <div class="input-append">
                                ...
                                <input class="m-wrap" type="text" name='phonenum' value='<?php echo (isset($keyword) && ($keyword !== '')?$keyword:''); ?>' placeholder='手机号码'>
                        ...
                        <br><br><br><br>
                        <!--<h4>当天获取数据条数为<?php echo (isset($count ) && ($count  !== '')?$count :'0'); ?>条&nbsp;&nbsp;&nbsp;&nbsp;      客服一:<?php echo (isset($count1 ) && ($count1  !== '')?$count1 :'0'); ?>条  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;      客服二:<?php echo (isset($count2 ) && ($count2  !== '')?$count2 :'0'); ?>条</h4>-->
                        <form action="<?php echo url('downLoad'); ?>" enctype="multipart/form-data" method="post" id='downLoad'>
                            <div class="control-group">
                                <div class="controls">
                                    ...
                                    <a href="javascript:;" id="daochu" class="btn blue downLoad_ffyue"><i class="icon-download-alt"></i>导出会员信息</a>
                        ...
                        <form method="post" action="<?php echo url('eximport'); ?>" enctype="multipart/form-data">
                            <h4>导入Excel表：</h4><input  type="file" name="file_stu" />
                        ...
                        <table class="table table-striped table-bordered table-hover table-full-width  table-condensed cf" id="sample_1">
                            ...
                            <tr data-id='<?php echo $info['id']; ?>'>
                                ...
                                <td data-title="客户名称" class="numeric"><?php echo (isset($info['nid']) && ($info['nid'] !== '')?$info['nid']:''); ?></td>
                                <td data-title="产品名称" class="numeric"><?php echo (isset($info['pid']) && ($info['pid'] !== '')?$info['pid']:''); ?></td>
                                <td data-title="负责人" class="numeric"><?php echo (isset($info['responsible']) && ($info['responsible'] !== '')?$info['responsible']:''); ?></td>
                                <td data-title="负责人部门" class="numeric"><?php echo (isset($info['res_dep']) && ($info['res_dep'] !== '')?$info['res_dep']:''); ?></td>
                                <td data-title="协作人" class="numeric"><?php echo (isset($info['collavorator']) && ($info['collavorator'] !== '')?$info['collavorator']:''); ?></td>
                                <td data-title="协作人部门" class="numeric"><?php echo (isset($info['coll_dep']) && ($info['coll_dep'] !== '')?$info['coll_dep']:''); ?></td>
                                <td data-title="任务进度" class="numeric"><?php echo (isset($info['task_progress']) && ($info['task_progress'] !== '')?$info['task_progress']:''); ?></td>
                                <td data-title="任务状态" class="numeric"><?php echo (isset($info['task_state']) && ($info['task_state'] !== '')?$info['task_state']:''); ?></td>
                                <td data-title="当前进度" class="numeric"><?php echo (isset($info['dq_progress']) && ($info['dq_progress'] !== '')?$info['dq_progress']:''); ?></td>
                                <td data-title="完成时间" class="numeric"><?php echo (isset($info['complete_time']) && ($info['complete_time'] !== '')?$info['complete_time']:''); ?></td>
                                <td data-title="服务金额" class="numeric"><?php echo (isset($info['service_price']) && ($info['service_price'] !== '')?$info['service_price']:''); ?></td>
                                <td data-title="支出预算" class="numeric"><?php echo (isset($info['expend_budget']) && ($info['expend_budget'] !== '')?$info['expend_budget']:''); ?></td>
                                <td data-title="回款进度" class="numeric"><?php echo (isset($info['coll_progress']) && ($info['coll_progress'] !== '')?$info['coll_progress']:''); ?></td>
                                <td data-title="末次收款时间" class="numeric"><?php echo (isset($info['last_coll_time']) && ($info['last_coll_time'] !== '')?$info['last_coll_time']:''); ?></td>
                                <td data-title="累计实收" class="numeric"><?php echo (isset($info['lj_sh']) && ($info['lj_sh'] !== '')?$info['lj_sh']:''); ?></td>
                                <td data-title="累计支出" class="numeric"><?php echo (isset($info['lj_zc']) && ($info['lj_zc'] !== '')?$info['lj_zc']:''); ?></td>
                                <td data-title="累计有效收入" class="numeric"><?php echo (isset($info['lj_yxsr']) && ($info['lj_yxsr'] !== '')?$info['lj_yxsr']:''); ?></td>
                                <td data-title="当期实收" class="numeric"><?php echo (isset($info['dq_sh']) && ($info['dq_sh'] !== '')?$info['dq_sh']:''); ?></td>
                                <td data-title="当期支出" class="numeric"><?php echo (isset($info['dq_zc']) && ($info['dq_zc'] !== '')?$info['dq_zc']:''); ?></td>
                                <td data-title="当期有效收入" class="numeric"><?php echo (isset($info['dq_yxsr']) && ($info['dq_yxsr'] !== '')?$info['dq_yxsr']:''); ?></td>
                                <td data-title="销售代表" class="numeric"><?php echo (isset($info['xsdb']) && ($info['xsdb'] !== '')?$info['xsdb']:''); ?></td>
                                <td data-title="销售代表部门" class="numeric"><?php echo (isset($info['xsdb_dep']) && ($info['xsdb_dep'] !== '')?$info['xsdb_dep']:''); ?></td>
                                <td data-title="本单来源分类" class="numeric"><?php echo (isset($info['ben_type']) && ($info['ben_type'] !== '')?$info['ben_type']:''); ?></td>
                                <td data-title="本单来源详情" class="numeric"><?php echo (isset($info['ben_detail']) && ($info['ben_detail'] !== '')?$info['ben_detail']:''); ?></td>
                                <td data-title="本单来源 来源人部门" class="numeric"><?php echo (isset($info['ben_lyrbm']) && ($info['ben_lyrbm'] !== '')?$info['ben_lyrbm']:''); ?></td>
                                <td data-title="本单来源 渠道负责人" class="numeric"><?php echo (isset($info['ben_qdfzr']) && ($info['ben_qdfzr'] !== '')?$info['ben_qdfzr']:''); ?></td>
                                <td data-title="本单来源负责人部门" class="numeric"><?php echo (isset($info['ben_fzrbm']) && ($info['ben_fzrbm'] !== '')?$info['ben_fzrbm']:''); ?></td>
                                <td data-title="本单来源 渠道类型" class="numeric"><?php echo (isset($info['ben_qdlx']) && ($info['ben_qdlx'] !== '')?$info['ben_qdlx']:''); ?></td>
                                <td data-title="服务公司" class="numeric"><?php echo (isset($info['service_company']) && ($info['service_company'] !== '')?$info['service_company']:''); ?></td>
                                <td data-title="创建时间" class="numeric"><?php echo (isset($info['create_time']) && ($info['create_time'] !== '')?$info['create_time']:''); ?></td>
                                <td data-title="首单创建时间" class="numeric"><?php echo (isset($info['first_c_time']) && ($info['first_c_time'] !== '')?$info['first_c_time']:''); ?></td>
                        ...
                        <div style="width: 1000px;height: 50px;position: relative;">
                            <label><input type="checkbox" class="group-checkable" data-set="#sample_1 .checkboxes" />全选</label>
