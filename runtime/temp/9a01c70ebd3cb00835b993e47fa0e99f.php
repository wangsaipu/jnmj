	...
	if($('li').hasClass('<?php echo $controller__action; ?>')){
		...
		<div class="page-content">
			<div class="container-fluid">
				...
				<a style="margin-bottom: 5px;" class="btn blue" target="_black" href="<?php echo url('add'); ?>"><i class="icon-plus"></i> 添加首页设置信息</a>
					...
					<div class="portlet box green">
						...
						<form action="<?php echo url('index'); ?>" id='searchForm' method='post' class="span3">
							...
							<input type="text" name="name"  class='m-wrap span8' placeholder="名称" value='<?php echo (isset($name) && ($name !== '')?$name:''); ?>'>
							...
							<table class="table table-striped table-bordered table-hover table-full-width table-condensed  cf"  id="sample_1" >
								...
								<tbody>
									...
									<tr data-id='<?php echo $info['id']; ?>' >
										...
										<td data-title="名称" class="numeric"><nobr><?php echo (isset($info['dy_name']) && ($info['dy_name'] !== '')?$info['dy_name']:''); ?></nobr></td>
										<td data-title="别名" class="numeric"><nobr><?php echo (isset($info['dy_bname']) && ($info['dy_bname'] !== '')?$info['dy_bname']:''); ?></nobr></td>
										<td data-title="标题" class="numeric"><nobr><?php echo (isset($info['dy_title']) && ($info['dy_title'] !== '')?$info['dy_title']:''); ?></nobr></td>
										<td data-title="描述" class="numeric inithidden"><nobr><?php echo (isset($info['dy_description']) && ($info['dy_description'] !== '')?$info['dy_description']:''); ?></nobr></td>
										<td data-title="关键词" class="numeric inithidden"><nobr><?php echo (isset($info['dy_keyword']) && ($info['dy_keyword'] !== '')?$info['dy_keyword']:''); ?>
										</nobr></td>
										<td data-title="APP二维码" class="numeric inithidden"><nobr>
										<img src="<?php echo (isset($info['dy_code']) && ($info['dy_code'] !== '')?$info['dy_code']:''); ?>" width="100px;">
										</nobr></td>
										<td data-title="qq分享图片" class="numeric inithidden"><nobr>
										<img src="<?php echo (isset($info['dy_qq']) && ($info['dy_qq'] !== '')?$info['dy_qq']:''); ?>" width="100px;">
										</nobr></td>
										<td data-title="微信分享图片" class="numeric inithidden"><nobr>
										<img src="<?php echo (isset($info['dy_weixin']) && ($info['dy_weixin'] !== '')?$info['dy_weixin']:''); ?>" width="100px;">
										</nobr></td>
...
$(function(){
	$('#delete').click(function(){
		...
		location.href = "<?php echo url('delete'); ?>?idArr="+idArr;
    });
});
</script>