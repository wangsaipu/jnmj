							...
							<div class="portlet-body form">
								<form action="<?php echo url('add'); ?>" method='post' id="form_sample_2" class="form-horizontal">
									...
									<div class="control-group">
										<label class="control-label">员工名<span class="required">*</span></label>
										<div class="controls">
											<select  class="span2 chosen" data-with-diselect="1" name="id" data-placeholder="员工名" tabindex="1">
												<option value=""></option>
						                        <?php echo new_getfkarr('name','cms_users',$arr,['status'=>1,'is_admin'=>0]); foreach($arr as $k=>$fk): if(isset($info['cms_users_id'])): ?>
						                        <option value="<?php echo $fk['id']; ?>" <?php if($info['cms_users_id']==$fk['id']): ?> selected <?php endif; ?>><?php echo $fk['name']; ?></option>
						                            <?php else: ?>
						                        <option value="<?php echo $fk['id']; ?>"><?php echo $fk['name']; ?></option>
						                            <?php endif; endforeach; ?>
											</select>
										</div>
