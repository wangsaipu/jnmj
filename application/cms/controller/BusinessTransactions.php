...
class BusinessTransactions extends CmsController
{
    public function index()
    {
        ...
        $res = db('salesum')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);
        //dump($res);exit;
        ...
        $arr = [ '员工名称', '所在部门', '代理记账', '社保公积金服务', '商标注册', '其他产品', '合同总额',
            '累计有效收入（毛利）','累计实收','累计支出'];
        $init_index = 2;
        if ($init_index == 2) {
