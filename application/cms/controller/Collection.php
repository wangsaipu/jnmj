...
class Collection extends CmsController
{
    public function index()
    {
        ...
        $res = db('collection')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);

        $this->ld();
        $this->assign('pagenumber', $res->total() - ($res->currentPage() - 1) * $res->listRows());
        $this->assign('list', $res);
        $this->assign('page', $res->render());
        $arr = ['应收账户', '账单流水号', '客户名称', '收款金额', '收款状态', '付款账户', '收据编号', '收据凭证', '备注', '收款人', '收款人部门'
            , '提交时间', '下次回访时间', '确认人', '确认人部门', '确认时间', '到账时间'];
        $init_index = 2;
        if($init_index ==2){
