...
class Expenditure extends CmsController
{
    public function index()
    {
        ...
        $res = db('expenditure')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);
        ...
        $arr = ['支出事项', '支出科目', '客户名称', '合同流水号', '产品名称', '实收', '可支出', '申请金额', '备注', '附件', '状态', '申请人', '申请时间',
            '出纳审核人', '支出时间', '付款公司', '付款账号'];
        $init_index = 2;
        if ($init_index == 2) {
