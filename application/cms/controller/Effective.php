...
class Effective extends CmsController
{
    public function index()
    {
        ...
        $res = db('effectiveincome')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);
        //dump($res);exit;
        ...
        $arr = [ '客户名称', '产品名称', '合同编号', '合同类型', '本单销售代表', '本单所属部门', '本单来源分类', '本单来源详情',
            '首单来源分类', '首单来源详情', '签订日期', '合同金额', '支出预算', '服务金额','月费用','回款比例','累计实收',
            '累计支出','累计有效收入','当期实收','当期支出','当期有效收入','服务公司'];
        $init_index = 2;
        if ($init_index == 2) {
