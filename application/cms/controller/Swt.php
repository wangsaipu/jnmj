...
class Swt extends cmsController
{
    public function index()
    {
        ...
        $res = db('excel_swt')->alias('a')->field('a.*,l.lname')->join($join)->where($where)->order('a.id desc')->paginate($searchSize, false, $post_val);
        foreach ($res as $k => $v) {
            if (!empty($v['phonenum'])) {

                $v['phone'] = (($v['phonenum'] - 5) / 3 + 6) / 8;
            } else {
                $v['phonenum'] = '';
            }
            $res[$k] = $v;
        ...
        $arr = ['序号', '姓名', '手机号码', 'QQ', '微信', '地域', '关键词', '来路URL', '项目', '时间', '分派时间', '客服', '数据状态', '备注', '作者'];
        $init_index = 2;
        if($init_index ==2){
...
//导入
    ...
    public function downLoad(){
        ...
        ob_end_clean();
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="'.$post['ff_yue'].'--客户信息-手机.xls"');
        header('Cache-Control: max-age=0');
        // If you're serving to IE 9, then the following may be needed
        header('Cache-Control: max-age=1');
        // If you're serving to IE over SSL, then the following may be needed
    ...
    public function add(){
        if($this->request->isPost()){
            ...
            $kefu = db('excel_swt')->field('phonenum')->where(array('phonenum' => ($post['phonenum']*8-6)*3+5))->select();
            if (empty($kefu)) {
                $post['read'] = '0';
                $post['author'] = session('cmsuser')['name'];
                $backurl = $post['backurl'];
                unset($post['backurl']);
                $post['username'] = empty($post['username']) ? '' : $post['username'];
                $post['phonenum'] = empty($post['phonenum']) ? '' : ($post['phonenum'] * 8 - 6) * 3 + 5;

                $post['keyword'] = empty($post['keyword']) ? '' : $post['keyword'];
                $post['inurl'] = empty($post['inurl']) ? '' : $post['inurl'];
                $post['area'] = empty($post['area']) ? '' : $post['area'];
                ...
                //var_dump($post);die;
                $data1 = db('excel_swt')->insert($post);
                if ($data1 !== false) {
            ...
            $this->assign('head_title', '添加商务通数据');
    ...
    public function update(){
        if ($this->request->isPost()) {
            ...
            try{
                ...
                //dump($post);die;
                db('excel_swt')->where(['id'=>$id])->update($post);
                Db::commit();
            }catch(\Exception $e){
                Db::rollback();
                $this->error('操作失败');
            ...
            $get = $this->request->param();
            $info = db('excel_swt')->where(['id'=>$get['id']])->find();
