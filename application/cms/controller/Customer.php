...
class Customer extends cmsController
{
    public function index()
    {
        ...
        $res = db('customer')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);
        ...
        $arr = ['客户名称', '微信群','联系人', '联系方式','客户负责人', '负责人部门','所有权人', '总合同额', '总代收','客户标签','客户来源类型','主营行业', '创建人',
            '创建部门','创建时间','注册行政区域','注册园区','社会信用代码','法定代表人','企业电话','企业邮箱','备注'];
        $init_index = 2;
        if($init_index ==2){
        ...
        $this->assign('hidden_index',[14,15,16,17,18,19,20,21]);//需要隐藏的列
        $this->assign('head_title','产品中心');
...
//导入
    ...
    public function add(){
        if($this->request->isPost()){
            ...
            if (empty($kefu)) {
                ...
                //客户编号
                $post['c_number'] = 'KH'.substr(date('YmdHis'),2).rand(1000,9999);
                $post['create_time'] = date('Y-m-d H:i:s', time());

                //dump($post);die;
                $data1 = db('customer')->insert($post);
                if ($data1 !== false) {
            ...
            //$this->assign('file_upload_index', ['wz_img' => '标题展示图(294*198)']);  //数据库中存储多文件的字段名,wz_img
    ...
    public function bank_info_add(){//添加客户银行信息
        $post = $this->request->param();
        $id = $post['id']; unset($id);
    }

    public function bank_info_update(){//更新客户银行信息
        $post = $this->request->param();
        $id = $post['id']; unset($id);
    }

    public function basics_info_add(){//更新客户工商信息
        $post = $this->request->param();
        $id = $post['id']; unset($id);
    }

    public function basics_info_update(){//更新客户工商信息
        $post = $this->request->param();
        $id = $post['id']; unset($id);
    }
