...
class Channel extends CmsController
{
    public function index()
    {
        ...
        $res = db('channel')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);
        //dump($res);exit;
        ...
        $arr = [ '渠道商名称', '联系人', '联系方式', '状态', '渠道负责人', '资源归属', '可见范围', '创建人','创建时间'];
        $init_index = 2;
        if ($init_index == 2) {
    ...
    public function add()
    {
        if ($this->request->isPost()) {
                ...
                $post['sign_date'] = date('Y-m-d');
                $post['create_time'] = date('Y-m-d H:i:s', time());

                //dump($post);die;
                $data1 = db('channel')->insert($post);
                if ($data1 !== false) {
    ...
    public function update()
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                //dump($post);die;
                db('channel')->where(['id' => $id])->update($post);
                Db::commit();
            } catch (\Exception $e) {
                Db::rollback();
                $this->error('操作失败');
            ...
            $get = $this->request->param();
            $info = db('channel')->where(['id' => $get['id']])->find();
