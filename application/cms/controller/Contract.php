...
class Contract extends cmsController
{
    public function index()
    {
        ...
        $res = db('contract')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);
        ...
        $arr = ['签约日期', '合同类型','合同状态', '合同金额','合同流水号', '合同编号', '合同来源', '客户名称', '创建人', '创建时间'];
        $init_index = 2;
        if($init_index ==2){
    ...
    public function add(){
        if($this->request->isPost()){
            ...
            if (empty($kefu)) {
                ...
                //合同编号
                $post['contract_number'] = 'HT'.substr(date('YmdHis'),2).rand(1000,9999);
                $post['create_time'] = date('Y-m-d H:i:s', time());

                //dump($post);die;
                $data1 = db('contract')->insert($post);
                if ($data1 !== false) {
