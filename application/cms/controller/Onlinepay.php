...
class Onlinepay extends CmsController
{
    public function index()
    {
        ...
        $res   = db('accounting')->alias('a')->field('a.*')->order('a.id desc')->paginate($searchSize, false, $post_val);
        $this->ld();
        //var_dump($kefu);die;
        $this->assign('pagenumber', $res->total() - ($res->currentPage() - 1) * $res->listRows());
        $this->assign('list', $res);
        $this->assign('page', $res->render());
        $arr = ['账单流水号', '创建时间', '收款公司', '支付方式', '费用类型', '付款状态', '退款状态', '到账时间', '交易人', '收款金额', '手续费','实际收款金额'];
        $init_index = 2;
        if($init_index ==2){
